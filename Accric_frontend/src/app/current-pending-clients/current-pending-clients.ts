...
@Component({
  selector: 'app-current-pending-clients',
  imports: [CommonModule, FormsModule],
  templateUrl: './current-pending-clients.html',
  styleUrl: './current-pending-clients.css'
})
export class CurrentPendingClients implements OnInit {
  search_text: string = "";
  ...
  ngOnInit(): void {
    this.loadCurrentPendingClients();
  }
  ...
  loadCurrentPendingClients(): void {
    ...
    this.http.get<{ data: AsvPendingResponse[], message: string }>(url, { headers }).subscribe({
      next: (response) => {
        this.isLoading = false;
        if (response.message === "Current ASV pending reports fetched successfully" && response.data) {
      ...
      error: (error) => {
        this.isLoading = false;
        console.error('Error loading current pending clients:', error);
  ...
  exportToExcel(): void {
    ...
    try {
      ...
      const fileName = `Current-Pending-Clients-${new Date().toISOString().split('T')[0]}.xlsx`;
      const ws: XLSX.WorkSheet = XLSX.utils.json_to_sheet(exportData);
      const wb: XLSX.WorkBook = {
        Sheets: { 'Pending Clients': ws },
        SheetNames: ['Pending Clients']
      };
